...
for (int i = 0; i < n; i++)
{
    int idx=upper_bound(ini(v1),v0[i])-v1.begin();
    idx--;
    if(idx>=0&&v1[idx]==v0[i]){
        ans+=i+n-i-1+idx+m-idx-1;
    }
}

for (int i = 0; i < n; i++)
{
     if(st1.find(v0[i]-1)!=st1.end()&&st1.find(v0[i]+1)!=st1.end()){
        ans++;
     }
}
for (int i = 0; i < m; i++)
{
     if(st0.find(v1[i]-1)!=st0.end()&&st0.find(v1[i]+1)!=st0.end()){
        ans++;
     }
}
